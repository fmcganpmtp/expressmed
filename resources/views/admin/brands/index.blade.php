        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <a href="{{ route('brands.create') }}" class="btn btn-success btn-circle btn-lg"><i class="fas fa-plus"></i></a>
                    ...
                    <div class="card-body">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            ...
                            @forelse ($data as $row)
                                <tr>
                                    <td>{{ ($data->currentpage() - 1) * $data->perpage() + $count + 1 }}</td>
                                    <td class="list-table-main-image">
                                        @if ($row->image != '')
                                            <img src="{{ asset('/assets/uploads/brands/' . $row->image) }}" class="img-thumbnail" />
                                        @else
                                    ...
                                    <td>
                                        <form action="{{ route('brands.destroy', $row->id) }}" method="POST">
                                            <a class="btn btn-warning btn-circle btn-md" href="{{ route('brands.edit', $row->id) }}"><i class="fas fa-pen"></i></a>
