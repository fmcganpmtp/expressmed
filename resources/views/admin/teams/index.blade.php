...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <a href="{{ route('teams.create') }}" class="btn btn-success btn-circle btn-lg"><i class="fas fa-plus"></i></a>
            <div class="card-body">
                    <table  class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @php $count=0; @endphp
                         @foreach($teams as $row)
                           <tr>
                            <td>{{ ($teams->currentpage() - 1) * $teams->perpage() + $count + 1 }}</td>
                                <td> {{$row->name}}</td>
                                <td><img src="{{ asset('/assets/uploads/teams/'.$row->image) }}" class="image-responsive" width="90" height="90" ></td>
                                <td>{{$row->position}}</td>
                                ...
                                <td  class="action_button_outer">
                                    <a href="{{url('/teams/edit/'.$row->id)}}" class="btn btn-warning btn-circle btn-md"><i class="fas fa-pen"></i></a>

                                    <form class="form-horizontal" method="POST" action="{{ route('teams.destroy',$row->id) }}">
                                    {{ csrf_field() }}
