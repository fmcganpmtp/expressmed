        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <a href="{{ route('medicineUse.create') }}" class="btn btn-success btn-circle btn-lg"><i class="fas fa-plus"></i></a>
                    ...
                    <div class="card-body">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            ...
                            @forelse ($MedicineUse as $key=> $row)
                                <tr>
                                    <td>{{ ($MedicineUse->currentpage() - 1) * $MedicineUse->perpage() + $key + 1 }}</td>
                                    <td>{{ $row->name }}</td>
                                    <td>
                                        <form action="{{ route('medicineUse.destroy', $row->id) }}" method="POST">
                                            <a class="btn btn-warning btn-circle btn-md" href="{{ route('medicineUse.edit', $row->id) }}"><i class="fas fa-pen"></i></a>
                                            @csrf
                        ...
                        {!! $MedicineUse->links('pagination::bootstrap-4')!!}
                    </div>
                </div>
            </div>
        </div>
