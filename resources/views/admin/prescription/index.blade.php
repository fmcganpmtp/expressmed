...
<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">Uploaded Prescriptions</h1>
    <div class="card shadow mb-4">
        <div class="card-body">
            <div class="table-responsive">
                <div id="myElem" class="alert alert-success float-right" style="display:none"></div>
                <div class="float-right">
                    <a href="{{ url('admin/prescription?status=active') }}" class="btn {{ ((empty(request()->all()) || request()->status == 'active' || (request()->status != 'approved' && request()->status != 'rejected' && request()->status != 'completed')) ? 'btn-success' : 'btn-primary') }}">Active</a>
                    <a href="{{ url('admin/prescription?status=approved') }}" class="btn {{ ((request()->status == 'approved') ? 'btn-success' : 'btn-primary') }}">Approved</a>
                    <a href="{{ url('admin/prescription?status=rejected') }}" class="btn {{ ((request()->status == 'rejected') ? 'btn-success' : 'btn-primary') }}">Rejected</a>
                    <a href="{{ url('admin/prescription?status=completed') }}" class="btn {{ ((request()->status == 'completed') ? 'btn-success' : 'btn-primary') }}">Completed</a>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td><a href="{{ route('shopping.productdetail', $row->product_url) }}" target="_blank">{{ ucwords($row->product_name) }}</a></td>
