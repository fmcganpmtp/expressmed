        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <a href="{{ route('productcontent.create') }}" class="btn btn-success btn-circle btn-lg"><i class="fas fa-plus"></i></a>
                    ...
                    <div class="card-body">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            ...
                            @forelse ($productcontents as $key=> $row)
                                <tr>
                                    <td>{{ ($productcontents->currentpage() - 1) * $productcontents->perpage() + $key + 1 }}</td>
                                    <td>{{ $row->name }}</td>
                                    <td>
                                        <form action="{{ route('productcontent.destroy', $row->id) }}" method="POST">
                                            <a class="btn btn-warning btn-circle btn-md" href="{{ route('productcontent.edit', $row->id) }}"><i class="fas fa-pen"></i></a>
                                            @csrf
                        ...
                        {!! $productcontents->links('pagination::bootstrap-4') !!}
                    </div>
                </div>
            </div>
        </div>
